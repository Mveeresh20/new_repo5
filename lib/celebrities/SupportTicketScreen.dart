...
class CelebritySupportTicketScreen extends StatefulWidget {
  const CelebritySupportTicketScreen({Key? key}) : super(key: key);

  @override
  _CelebritySupportTicketScreenState createState() => _CelebritySupportTicketScreenState();
}

class _CelebritySupportTicketScreenState extends State<CelebritySupportTicketScreen> {
  ...
  final String userType = "celebrity";
  ...
  Future<void> fetchUserData(String u_id) async {
    final String getBasicDetailsApi = "https://demo.infoskaters.com/api/get_celebrity_basic.php?c_id=$u_id";

    try {
      final response = await http.get(Uri.parse(getBasicDetailsApi));
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          ...
          _mobileController.text = data['c_mob'] ?? '';
        });
      } else {
